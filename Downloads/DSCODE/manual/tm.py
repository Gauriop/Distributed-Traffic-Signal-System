...
def register_time_and_sync():
    ...
    print("🧪 ENHANCED: Load Test with proper error handling and timeouts")
...
def single_load_test_request(request_id):
    """Execute a single request for load testing with proper error handling"""
    ...
    try:
        ...
        # Create VIP request with unique timestamp
        current_time = int(time.time())
        base_time = 1700000000
        relative_timestamp = current_time - base_time + request_id * 100  # More unique timestamp
        ...
        # Submit VIP request
        submit_success = thread_server.submit_vip_requests(vip_data)
        if not submit_success:
            return f"Request #{request_id} -> Route {route}: SUBMIT FAILED"
        
        # Process VIP request
        process_success = thread_server.vip_signal_manipulator(route)
        
        end_time = time.time()
        duration = end_time - start_time
        
        if process_success:
            return f"Request #{request_id} -> Route {route}: SUCCESS ({duration:.2f}s)"
        else:
            return f"Request #{request_id} -> Route {route}: PROCESS FAILED ({duration:.2f}s)"
...
def run_enhanced_load_test():
    """Run enhanced load test with better error handling and monitoring"""
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        ...
        print(f"\n🚀 LAUNCHING 15 SIMULTANEOUS REQUESTS WITH ENHANCED ERROR HANDLING...")
...
def show_menu():
    ...
    print("\n" + "=" * 70)
    print("👑 ENHANCED MANUAL VIP CONTROLLER MENU - WITH ROBUST LOAD TESTING")
    print("=" * 70)
    ...
    print("8. 🧪 RUN ENHANCED LOAD TEST (15 requests with error handling)")
    print("9. ❌ Exit")
    print("=" * 70)
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                confirm = input("🧪 Are you sure you want to run the enhanced load test? (y/n): ").strip().lower()
                if confirm in ['y', 'yes']:
